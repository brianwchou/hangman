...
contract('Hangman', async (accounts) => {
    ...
    describe("Test game with no solution set", async () => {
        ...
        it("check solution can be set if there is no solution", async() => {
            await truffleAssert.passes(hangmanContract.setSolution(web3.fromAscii("hello"), 5), "should be able to set solution");
        });
        ...
        it("check makeCharGuess", async () => {
            await truffleAssert.reverts(hangmanContract.makeCharGuess(web3.fromAscii("e")), "solution not set");
        });

        it("check makeWordGuess", async () => {
            await truffleAssert.reverts(hangmanContract.makeWordGuess(web3.fromAscii("world")), "solution not set")
        });
