...
contract('StartGame', async (accounts) => {
  ...
  before('deploy StartGame', async() => {
      ...
      //mock LinkToken.sendChainlinkRequest()
      let mockLink_sendChainlinkRequest = 
        linkTokenTemplate.contract.methods.sendChainlinkRequest(0, 0).encodeABI();
      await mockLinkToken.givenCalldataReturn(mockLink_sendChainlinkRequest, "testing");
